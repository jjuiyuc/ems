...
CREATE TABLE `group_type_webpage_right` (
  ...
  `create_data` tinyint(1) DEFAULT '0',
  `read_data` tinyint(1) DEFAULT '0',
  `update_data` tinyint(1) DEFAULT '0',
  `delete_data` tinyint(1) DEFAULT '0',
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
...
CREATE TABLE `group_gateway_right` (
  ...
  KEY `group_gateway_right_location_id_location_id_foreign` (`location_id`),
  ...
  CONSTRAINT `group_gateway_right_location_id_location_id_foreign` FOREIGN KEY (`location_id`) REFERENCES `location` (`id`) ON DELETE RESTRICT ON UPDATE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
